...
# ── Scenario 4: 缺少 API Key 时启动报错 ───────────────────────────────


@pytest.mark.feature
@pytest.mark.slow
class TestScenario4_MissingApiKey:
    """清除所有 API Key 环境变量，验证启动时抛出清晰错误信息."""

    PORT = 18704

    def test_service_fails_to_start_without_api_key(self):
        """Service exits with error when no API key is set."""
        # Build clean env without any LLM keys
        clean_env = {
            k: v
        ...
        with pytest.raises(RuntimeError) as exc_info:
            _start_service(self.PORT, env=clean_env, timeout=15.0)

        error_msg = str(exc_info.value)
        # Should contain LLM config error or reference to missing env var
        assert (
            "LLM 配置错误" in error_msg
            or "MAAS_API_KEY" in error_msg
            or "MODEL_API_KEY" in error_msg
        ), (
            f"Expected clear error about missing API key/LLM config, got: {error_msg}"
        )

    def test_service_fails_when_maas_key_specifically_missing(self):
        """When config.yaml specifies maas but MAAS_API_KEY is unset, error names MAAS_API_KEY."""
        clean_env = {
            k: v
            for k, v in os.environ.items()
            if k not in ("MAAS_API_KEY", "DEEPSEEK_API_KEY", "MODEL_API_KEY")
        }
        # Setting DEEPSEEK_API_KEY shouldn't help because default is maas
        clean_env["DEEPSEEK_API_KEY"] = "irrelevant-key"

        with pytest.raises(RuntimeError) as exc_info:
            _start_service(self.PORT, env=clean_env, timeout=15.0)

        error_msg = str(exc_info.value)
        assert "MAAS_API_KEY" in error_msg or "LLM 配置错误" in error_msg, (
            f"Error should reference MAAS_API_KEY, got: {error_msg}"
        )

    def test_fallback_fails_when_model_api_key_missing(self):
        """When config.yaml is absent and MODEL_API_KEY unset (and .env disabled), service fails."""
        # Remove config.yaml
        if CONFIG_YAML.exists():
            CONFIG_YAML.unlink()
        # Also disable .env file (which would otherwise provide MODEL_API_KEY)
        if DOTENV_FILE.exists():
            DOTENV_FILE.rename(DOTENV_BACKUP)
        ...
        try:
            ...
            error_msg = str(exc_info.value)
            assert "MODEL_API_KEY" in error_msg or "LLM 配置错误" in error_msg, (
                f"Expected error about MODEL_API_KEY, got: {error_msg}"
            )
        finally:
            _restore_config()
            # Restore .env file
            if DOTENV_BACKUP.exists():
                DOTENV_BACKUP.rename(DOTENV_FILE)
