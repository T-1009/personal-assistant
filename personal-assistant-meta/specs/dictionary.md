...
| **GitHub MCP Activity Data Source** | Service 工程活动数据源。通过 AgentArts MCP Gateway 调用 GitHub remote MCP，查询 `commit`、`pull_request`、`issue`、`review`、`comment` 五类活动，供 Report 和只读 Chat inspection 使用。它使用 platform GitHub account，不代表当前 Web Chat 用户；Agent 只能调用经过裁剪的 wrapper，不能调用 raw MCP passthrough |
...
| **GitHub MCP Activity Source** | Agent data source，使用 AgentArts MCP Gateway + GitHub remote MCP 读取平台账号可见的工程活动。与 GitHub Tools 不同，它不使用当前用户 OAuth token。当前默认启用四个只读 Chat inspection wrappers；设置 `GITHUB_MCP_CHAT_TOOL_ENABLED=false` 可只保留内部调用 |
