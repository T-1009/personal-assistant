...
| `personal-assistant-service/.agentarts_config.yaml` | 删除旧版 `MODEL_API_KEY` / `MODEL_NAME` / `MODEL_URL` env var；新增 `MAAS_API_KEY` env var 以匹配 config.yaml 中 maas provider 的 `api_key_env` |
