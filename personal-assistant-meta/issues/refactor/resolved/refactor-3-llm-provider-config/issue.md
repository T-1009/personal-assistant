---
status: superseded
superseded_by: refactor-10-centralize-runtime-settings
---

# Refactor 3: 重构 LLM Provider 静态配置体系

> **Superseded**：本 issue 不再单独实施。其 Provider 配置完善、默认 Provider
> 调整和 legacy 环境变量清理范围，已由
> [Refactor 10: 统一配置入口并清理遗留配置体系](../../resolved/refactor-10-centralize-runtime-settings/issue.md)
> 吸收并按“`.env.example` 是唯一用户配置入口”的原则重新设计。以下内容仅保留为
> 历史设计记录，不再作为 Implementation 输入。
