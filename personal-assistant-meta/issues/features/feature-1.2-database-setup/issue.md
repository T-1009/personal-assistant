...
数据库选型已在 ADR-012 确定为 PostgreSQL 17。

## 范围

- Docker Compose 配置（本地开发：`postgres:17-alpine`）
...
- 生产 RDS apply（Feature 9 一起做；OpenTofu 配置可提前编写和执行
  `tofu plan`，但 apply 延后）
...
- `ADR-012` — 持久化数据库选型（PostgreSQL 17）
- `ADR-006` — IaC 工具选型（生产 RDS 部署走 OpenTofu + HCL）
