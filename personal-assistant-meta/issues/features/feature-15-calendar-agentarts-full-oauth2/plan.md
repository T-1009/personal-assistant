...
再由可信后端调用 `complete_resource_token_auth(session_uri, user_id)` 完成 Resource Token
...
sequenceDiagram
    ...
    Agent->>Agent: 设置 user_id/session_id/workload token/callback_url/custom_state
    ...
    Agent->>Agent: 从 Gateway header 取可信 user_id，校验 state/session_uri
    Agent->>IdSvc: complete_resource_token_auth(session_uri, UserIdentifier(user_id))
    IdSvc->>MS: code exchange
    IdSvc->>IdSvc: 保存 Calendar Resource Token
    Agent-->>CB: complete success
    CB-->>UI: postMessage(auth_complete)
...
| URL | 所在层 | 调用者 | 职责 |
|-----|--------|--------|------|
| `/auth/callback/m365-calendar` | Client | Microsoft / browser redirect | 展示授权结果，读取 `session_uri` / `state` / error query params，调用后端 complete API |
...
| Service Tests | `tests/test_oauth2_complete.py` | 新建 | complete endpoint / state / replay / trusted user_id 测试 |
...
MVP 推荐使用 signed state + 短 TTL in-memory replay guard：
...
flowchart LR
    ...
    Complete --> Verify["校验签名、exp、provider、user_id、nonce replay"]
    Verify --> IdSvc["complete_resource_token_auth"]
```

实现要点：

- `state` 必须绑定可信 `user_id`、`session_id`、provider、nonce、expiry。
- complete endpoint 从 Gateway header 取可信 `user_id`，与 signed state 中的 `user_id` 比对。
- in-memory used-nonce store 用于 replay 防护；重复 callback 若已 complete，可返回 idempotent success。
- 若未来 AgentArts Runtime 多副本部署，in-memory replay guard 需要迁移到外部 store；本 Feature 将该点记录为 known limitation。
...
- 缺少可信 Gateway user header 返回 401。
...
该 callback page 仍包在 `MsalProvider` / `AuthGuard` 外层语境中时需谨慎：callback complete 需要可信登录态/Gateway header。若 `AuthGuard` 会拦截 callback page，应让 callback page 在 App 顶层优先渲染，并在 page 内等待 auth hydration 后调用 complete API。
...
- complete 成功后展示“授权完成，可以关闭窗口”，并通知 opener：

```js
window.opener?.postMessage(
  { type: "m365-calendar-auth", status: "complete", provider: "m365-calendar-provider" },
...
- 复用现有 auth token 发送机制。如果现有 `invokeChat` 的 header 构造只封装在 chat API client 中，应提取 shared `buildAuthHeaders()`，避免 callback complete 漏带 Authorization。
...
- 缺少 Gateway user header → 401。
- 缺少 `session_uri` → 400。
- 缺少/非法/过期 `state` → 403。
- body 伪造 `user_id` 不影响可信 user 校验。
- user mismatch → 403。
- valid request 调用 `IdentityClient.complete_resource_token_auth`，参数为 `UserIdentifier(user_id=trusted_user_id)`。
...
- callback page success：query params → POST complete → success UI → `postMessage(auth_complete)`。
