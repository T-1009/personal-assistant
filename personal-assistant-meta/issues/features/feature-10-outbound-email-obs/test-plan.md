...
| UT-TI-02 | `test_build_tools_includes_email_tools` | 验证 EMAIL_TOOLS 中的 5 个函数都在返回列表中 | 列表包含 `list_emails`, `get_email`, `search_emails`, `send_email`, `draft_reply` | patch `_init_provider` 避免真实 IdentityClient 调用 |
...
#### 1.2.6 `draft_reply` 测试

| # | 测试函数 | 场景 | 输入 | Mock Graph API 响应 | 预期输出 |
|---|---------|------|------|---------------------|---------|
| UT-DR-01 | `test_draft_reply_returns_draft_info` | 草拟回复成功 | `email_id="msg-1", body="Thanks", access_token="mock_token"` | `201 Created: {"id": "draft-1", "subject": "RE: Hello", "body": {"content": "Thanks"}, "toRecipients": [{"emailAddress": {"name": "Alice", "address": "alice@example.com"}}]}` | `{"draft_id": "draft-1", "subject": "RE: Hello", "body": "Thanks", "toRecipients": [...], "error": None}` |
| UT-DR-02 | `test_draft_reply_failure` | 草拟失败 | 同上 | `400 Bad Request`，非 201 | `{"draft_id": None, "subject": "", "body": "Thanks", "toRecipients": [], "error": ...}` |
| UT-DR-03 | `test_draft_reply_calls_create_reply_endpoint` | 验证调用 `/messages/{id}/createReply` | `email_id="msg-1"` | `201 Created` | URL 为 `/me/messages/msg-1/createReply` |
| UT-DR-04 | `test_draft_reply_no_access_token` | access_token 为 None 时 | `access_token=None` | 预期 Bearer header 为 `Bearer None`（装饰器 passthrough 后显式传入） | 可调用（Bearer None），由 Graph API 返回 401 |

#### 1.2.7 Provider 初始化测试
...
| UT-PI-03 | `test_provider_init_only_once` | 重复 import 不会重复创建 provider | 设置 env var | mock IdentityClient，`_init_provider()` 两次调用 | `create_oauth2_credential_provider` 仅调用一次 |
...
| UT-AH-02 | `test_system_prompt_mentions_email_capabilities` | SYSTEM_PROMPT 包含邮件能力描述 | `SYSTEM_PROMPT` 字符串包含 `list_emails`, `send_email`, `draft_reply`, `search_emails`, `get_email` 关键词 | 静态字符串断言 |
...
| IT-04 | `test_invocation_draft_reply_flow` | 用户请求回复邮件 → Agent 调用 `draft_reply` 展示草稿 | `POST /invocations {"message": "帮我回张三的邮件说收到", "stream": false}` | Agent 先返回草稿预览（含收件人、主题、正文），不直接发送 | `@pytest.mark.slow` |
| IT-05 | `test_invocation_guard_confirm_send` | 草稿展示后用户确认发送 → Agent 调用 `send_email` | 第一轮：请求回复 → 获取草稿。第二轮：`"发送"` | 第二轮返回 "邮件已发送" 或类似确认消息 | `@pytest.mark.slow` |
| IT-06 | `test_invocation_guard_cancel` | 草稿展示后用户取消 → Agent 不发送 | 第一轮：请求回复 → 获取草稿。第二轮：`"取消"` | Agent 确认取消，不调用 `send_email` | `@pytest.mark.slow` |
...
| # | 测试函数 | 场景 | 步骤 | 预期结果 |
|---|---------|------|------|---------|
| E2E-04 | `test_draft_reply_guard_flow` | 用户请求回复 → Agent 展示草稿 → 用户确认 → 发送成功 | **Round 1**: `POST /invocations {"message": "帮我回张三最近那封邮件，说收到"}`<br/>→ 验证 Agent 返回草稿预览且**未**发送<br/>**Round 2** (同 session): `POST /invocations {"message": "发送"}`<br/>→ 验证 Agent 确认发送成功 | Round 1: `response` 含收件人/主题/正文预览，不含 "已发送"<br/>Round 2: `response` 含 "已发送"/"发送成功" |
| E2E-05 | `test_draft_reply_cancel_flow` | 用户请求回复 → Agent 展示草稿 → 用户取消 | **Round 1**: 同上<br/>**Round 2**: `POST /invocations {"message": "取消"}` | Round 2: `response` 含 "已取消"/"不发送"，不应含 "已发送" |
| E2E-06 | `test_direct_send_shows_preview_first` | 用户直接指定收件人和内容 → Agent 先展示预览 | `POST /invocations {"message": "帮zhangsan@example.com发邮件说你好"}` | `response` 先展示预览，含 "需要发送吗？"/"确认" 等确认请求，不含 "已发送" |
...
| E2E-08 | `test_cross_session_with_canceled_message` | Session 1 中取消发送的邮件不影响 Session 2 | 同 session-id-A 内完成草拟→取消流程后，新 session 查询 | 新 session 正常查邮件，不受前 session 取消操作影响 |
...
| `app/tools/email_tools.py` | ≥ 90% | 同上 | 5 个工具函数 + `_init_provider()` |
...
flowchart TB
    subgraph Backend["🔧 后端测试 (personal-assistant-service/tests/)"]
        ...
        subgraph Unit["Unit Tests"]
            ...
            ET["test_email_tools.py<br/>~25 tests:<br/>• list_emails (6)<br/>• get_email (4)<br/>• search_emails (5)<br/>• send_email (6)<br/>• draft_reply (4)<br/>• Provider init (4)"]
            AH["test_agent_handler.py<br/>+3 tests:<br/>• tools 注入验证<br/>• SYSTEM_PROMPT 邮件能力<br/>• SYSTEM_PROMPT Guard 措辞"]
        end

        subgraph Integration["Integration Tests"]
            INT["test_email_integration.py<br/>8 tests:<br/>• 查看收件箱<br/>• 搜索邮件<br/>• 获取详情<br/>• 草拟回复<br/>• Guard 确认/取消<br/>• 直接发送预览<br/>• 未授权处理"]
        end
    end
...
| 优先级 | 测试范围 | 理由 |
|--------|---------|------|
| **P0 (Blocker)** | UT-LE-01~06, UT-GE-01~04, UT-SE-01~05, UT-SND-01~06, UT-DR-01~04 | 核心邮件工具函数单元测试 — 5 个工具函数的正确性是整个 Feature 的基石 |
