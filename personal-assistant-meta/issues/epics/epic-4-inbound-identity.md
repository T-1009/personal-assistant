...
飞书和 OfficeClaw 渠道自带用户身份（飞书 user_id）。但 Web Chat 和 User Federation 工具（GitHub/Google）需要用户通过 OAuth 登录，Agent 拿到用户身份后才能以用户身份调外部 API。本 Phase 搭建 OAuth 基础设施，Web Chat 渠道本身放到 Epic 5。
...
- SSE 流式对话（Epic 5）
- Web Chat 前端页面（Epic 5）
- Outbound 工具（Epic 6-8）
...
- [ ] `handle()` 中 user_id 从以下来源获取：
  ...
  - Cookie 中的 id_token（Web Chat，Epic 5 接入）
...
- Epic 1（Agent 骨架 + 飞书）
- Epic 2（Memory）— user_id 需关联 Memory
