...
> **认证代理**：Gateway 要求 `Authorization: Bearer` header（`authorizer_type: API_KEY`），浏览器不能持有 API Key。当前通过 Netlify Edge Function 在服务端注入认证 header 后转发，详见 [ADR-014](../ADR/ADR-014-netlify-edge-function-auth-proxy.md)。
