...
`psycopg.OperationalError` 在 `personal-assistant-service/tests/` 覆盖 preflight restart、
single retry，以及 Agent execution 启动后不 retry。E2E 只需复用既有 SSE/full-stack
测试确认跨边界 contract 未改变；不在 E2E 中伪造 Runtime 内部数据库连接故障。
