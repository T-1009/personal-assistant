...
flowchart TB
    ...
    subgraph HuaweiCloud["华为云 (cn-southwest-2)"]
        ...
        subgraph AgentArts["AgentArts 平台"]
            FastAPI["FastAPI 容器 :8080<br/>─────────────────<br/>/ping<br/>/invocations<br/>/invocations/stream<br/>/invocations/playground<br/>/feishu/webhook<br/>/auth/callback"]
        end
    end
...
sequenceDiagram
    ...
    HC->>FastAPI: GET /invocations/stream?q=ping
    FastAPI->>LLM: 请求推理
...
| 验证项 | 传统 `/health` | 聊天式 `/invocations/stream` |
|--------|:---:|:---:|
| FastAPI 进程存活 | ✅ | ✅ |
| LLM API 连通性 | ❌ | ✅ |
| SSE 流式中间件正常 | ❌ | ✅ |
