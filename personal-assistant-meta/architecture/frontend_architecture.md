...
Chainlit 定位为**同容器唯一的对话 UI**，挂载在 `/playground`，覆盖开发调试、Agent 链路验证和运维访问需求：

```
FastAPI 容器 :8080
  ├── /playground          → Chainlit app（对话/调试 UI）
  ├── /api/*               → 业务 API 路由
  └── ...                  → 其他路由
```
...
Web Chat 前端独立部署于华为云 OBS，不打包进 FastAPI 容器。同容器的对话 UI 需求由 Chainlit Playground（`/playground`）覆盖。
...
FastAPI 容器内仅保留 Chainlit Playground（`/playground`）作为对话 UI，覆盖以下场景：

| 用途 | 说明 |
|------|------|
| **开发调试** | 本地 `uvicorn` 启动后直接访问 `/playground`，零构建验证 Agent 链路 |
